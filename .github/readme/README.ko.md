...
대상은 넷이고, 각각 규칙이 하나입니다. 코드, 명령, 파일 경로, 식별자, 오류
메시지는 글자 하나까지 그대로 옮깁니다. 인용문도 그대로 옮깁니다. 코드 주석과
커밋 메시지는 그 저장소의 문체를 따릅니다. 이 스타일을 따르는 것은 에이전트가
직접 쓰는 산문뿐입니다.

이것은 지시이지 잠금장치가 아닙니다. 출력 스타일은 시스템 프롬프트 텍스트이고,
모델 바깥에서 이를 강제하는 장치는 없습니다. 이 경계는 강한 기본값으로 보고,
중요한 경우에는 결과를 확인하세요.
...
24개 사례, 6개 채점 항목, 그리고 정확성이나 안전성 퇴보를 막는 릴리스 게이트.
